...
**调用流程：** Java 后端收到前端 Word 生成请求 → 转发到本接口 → AI 模块执行生成（RAG 检索 → LLM 内容生成 → python-docx
...
**调用流程：** Java 后端收到前端 PDF 生成请求 → 转发到本接口 → AI 模块执行生成（RAG 检索 → LLM 内容生成 → python-docx
构建 → LibreOffice 转 PDF） → 上传文件到 Java 存储 → 返回结果。
