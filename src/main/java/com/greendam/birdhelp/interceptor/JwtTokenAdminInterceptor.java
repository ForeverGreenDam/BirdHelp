...
/**
 * <p>
 * 管理员 JWT 令牌校验拦截器，在请求到达 Controller 之前对 {@code /admin/**} 路径进行鉴权。
 * </p>
 *
 * <h3>拦截流程</h3>
 * <ol>
 *   <li>判断当前拦截目标是否为 Controller 方法，非动态资源直接放行</li>
 *   <li>从请求头 {@code admin-token} 中获取管理员 JWT 令牌</li>
 *   <li>使用 {@code adminSecretKey} 解析令牌，提取 {@code id} 声明作为当前管理员 ID</li>
 *   <li>将管理员 ID 存入 {@link BaseContext} 的 ThreadLocal 中，供后续处理使用</li>
 *   <li>请求结束后在 {@link #afterCompletion} 中清理 ThreadLocal，防止内存泄漏</li>
 * </ol>
 *
 * <h3>与用户端拦截器的区别</h3>
 * <ul>
 *   <li>使用独立的 {@code adminSecretKey} 签发/校验 JWT（与用户端 {@code userSecretKey} 隔离）</li>
 *   <li>读取独立的请求头 {@code admin-token}（与用户端 {@code token} 隔离）</li>
 *   <li>仅拦截 {@code /admin/**} 路径，与用户端拦截器路径不重叠</li>
 * </ul>
 *
 * <h3>鉴权失败处理</h3>
...
public class JwtTokenAdminInterceptor implements HandlerInterceptor {
    ...
    /**
     * <p>Controller 方法调用前执行，完成管理员 JWT 校验和身份注入。</p>
     *
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        try {
            ...
            try {
                ...
                log.error("写入管理员JWT错误响应失败", writeEx);
            }
            return false;
        }
    }
